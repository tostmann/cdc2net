...
static void deferred_ap_teardown_task(void *arg)
{
    ...
    // ANY uplink counts, not just the STA: on the W5500 generation the captive
    // AP is torn down by ethernet coming back just as well, and there s_connected
    // (a WiFi-STA flag) stays false.  Re-checked after the grace period so a
    // link that flapped away again leaves the AP in place.
    if (net_uplink_is_up() && s_ap_active) {
        net_stop_softap();
    }
...
static void net_stop_softap(void)
{
    ...
    ESP_LOGW(TAG, "tearing down captive AP — uplink is back, AP no longer needed");
