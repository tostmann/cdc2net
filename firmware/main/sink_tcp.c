...
static void replay_flush_to(tcp_client_t *c);   // Reboot-Nachlauf, Definition unten
...
static void tn_subneg(tcp_client_t *c)
{
    ...
    case CPO_SET_CONTROL: {
        ...
        // AVR-bootloader sticks.  Echo a sensible state.
...
static void accept_task(void *arg)
{
    ...
    while (1) {
        ...
        slot->active        = true;
        S.stats.active_clients++;
        // Erst das, was das Radio waehrend der Reconnect-Luecke gesagt hat
        // (Boot-Ready-Frame nach einem Reboot) — sonst wartet der Host
        // vergeblich auf die Antwort zu seinem NCP_RESET.
        replay_flush_to(slot);
        xSemaphoreGive(S.mtx);
...
// ── Reboot-Nachlaufpuffer ─────────────────────────────────────────────────
// Nach einem Radio-Reboot trennen wir die Clients (siehe sink_tcp_drop_clients),
// aber das Radio meldet sich ~1,5 s spaeter mit seinem Boot-Ready-Frame — und
// GENAU der ist die Antwort, auf die der Host nach seinem NCP_RESET wartet.
// zigbee-herdsman reconnectet erst nach 3 s, der Frame fiele also in die Luecke
// und der Host liefe in einen 10-s-Timeout ("Failure send 2").  In der
// wifi-coex-Firmware passiert das nicht, weil der Coordinator dort nach dem
// Reboot erst WiFi hochfahren muss und damit langsamer ist als der Host.
// Deshalb: fuer ein kurzes Fenster nach dem Drop mitschreiben und dem naechsten
// Client zuerst ausliefern.  Klein gehalten — es geht um Boot-Frames, nicht um
// eine Session-Wiederherstellung; laeuft der Puffer voll, gewinnt das Neueste.
#define REPLAY_BUF_SZ     512
#define REPLAY_WINDOW_MS  8000

static uint8_t  s_replay[REPLAY_BUF_SZ];
static size_t   s_replay_len;
static int64_t  s_replay_until_us;      // 0 = Fenster zu

static void replay_arm(void)
{
    s_replay_len     = 0;
    s_replay_until_us = esp_timer_get_time() + (int64_t)REPLAY_WINDOW_MS * 1000;
}

static void replay_capture(const uint8_t *data, size_t len)
{
    if (!s_replay_until_us) return;
    if (esp_timer_get_time() > s_replay_until_us) {     // Fenster abgelaufen
        s_replay_until_us = 0;
        s_replay_len      = 0;
        return;
    }
    for (size_t i = 0; i < len; i++) {                  // neueste Bytes gewinnen
        if (s_replay_len < REPLAY_BUF_SZ) {
            s_replay[s_replay_len++] = data[i];
        } else {
            memmove(s_replay, s_replay + 1, REPLAY_BUF_SZ - 1);
            s_replay[REPLAY_BUF_SZ - 1] = data[i];
        }
    }
}

// Vom accept-Pfad aufgerufen, solange der Mutex gehalten wird.
static void replay_flush_to(tcp_client_t *c)
{
    if (!s_replay_until_us || !s_replay_len) return;
    if (esp_timer_get_time() <= s_replay_until_us) {
        ESP_LOGI(TAG, "replaying %u byte(s) buffered across the radio reboot",
                 (unsigned)s_replay_len);
        client_send(c, s_replay, s_replay_len);
    }
    s_replay_until_us = 0;
    s_replay_len      = 0;
}
...
static void on_rx(sink_t *s, const uint8_t *data, size_t len)
{
    ...
    if (!fanned) replay_capture(data, len);   // niemand da → fuer den naechsten aufheben
    xSemaphoreGive(S.mtx);
}
...
// Gemeinsamer Kern für "alle Clients trennen"; Aufrufer liefert nur den Grund
// fürs Log.  Siehe sink_tcp.h für den inReset-Hintergrund.
static int drop_all_clients_locked(void)
{
    int dropped = 0;
    S.ctrl_client = NULL;
    for (int i = 0; i < SINK_TCP_MAX_CLIENTS; i++) {
...
void sink_tcp_drop_clients(const char *reason)
{
    if (!S.mtx) return;                      // sink noch nicht initialisiert
    xSemaphoreTake(S.mtx, portMAX_DELAY);
    int dropped = drop_all_clients_locked();
    replay_arm();                              // Boot-Frame des Radios auffangen
    xSemaphoreGive(S.mtx);
    if (dropped) {
        ESP_LOGW(TAG, "%s → dropping %d client(s) so the host reconnects",
                 reason ? reason : "state invalidated", dropped);
    }
}
