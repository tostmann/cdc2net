...
// Alle Client-Verbindungen droppen (FIN), damit der Host den Abbruch SIEHT und
// neu verbindet.  Gedacht für Ereignisse, die den Zustand hinter der Bridge
// entwerten — v.a. ein Reboot des angeschlossenen Radios.
//
// Warum das nötig ist: zigbee-herdsman setzt beim NCP-Reset ein `inReset`-Flag
// und löscht es NUR in `onPortClose`.  Bleibt die TCP-Session über den Reboot
// des Radios bestehen (bei uns der Normalfall — der Socket endet ja am S3, nicht
// am H2), feuert dort nie ein Close, `inReset` bleibt gesetzt, und
// `zbossAdapter.permitJoin()` fällt danach still durch seinen `if
// (driver.isInitialized())`-Block: kein ZDO-Permit, keine Exception, aber auch
// kein Gerät kommt mehr ins Netz.  Ein FIN von hier macht daraus ein echtes
// Port-Close.  Gleiches Mittel wie in der wifi-coex-Firmware, die vor
// `esp_restart()` selbst `transport::close_tcp_client()` ruft — nur muss es hier
// die Bridge tun, weil das Radio von der TCP-Verbindung nichts weiß.
//
// Nur `shutdown()`; das `close()` bleibt beim jeweiligen client_task (kein
// Double-Close / fd-Race).  No-op ohne verbundene Clients.
